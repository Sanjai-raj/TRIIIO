...
console.log('CLOUDINARY env presence:', !!process.env.CLOUDINARY_CLOUD_NAME, !!process.env.CLOUDINARY_API_KEY, !!process.env.CLOUDINARY_API_SECRET);
console.log('CLOUDINARY values (masked):', 'cloud_name=', mask(process.env.CLOUDINARY_CLOUD_NAME), 'api_key=', mask(process.env.CLOUDINARY_API_KEY), 'api_secret=', mask(process.env.CLOUDINARY_API_SECRET));
...
app.post("/api/products", auth, owner, upload.array("images", 5), async (req, res) => {
    try {
        const { name, description, price, discount, category, brand, stock, colors, variants, tags, } = req.body;
        // Upload images
        const imageResults = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
...
app.put('/api/products/:id', auth, owner, upload.array('images', 5), async (req, res) => {
    try {
        const product = await Product.findById(req.params.id);
        if (!product) {
        ...
        const removedImages = product.images.filter((img) => !existingImages.some((e) => e.publicId === img.publicId));
        ...
        if (files && files.length > 0) {
            ...
            const imageResults = await Promise.all(files.map(file => uploadToCloudinary(file.buffer)));
...
app.delete('/api/products/:id', auth, owner, async (req, res) => {
    try {
...
app.get('/api/orders', auth, owner, async (req, res) => {
    try {
...
app.put('/api/orders/:id', auth, owner, async (req, res) => {
    try {
...
// List Users
app.get('/api/admin/users', auth, owner, async (req, res) => {
    try {
...
app.put('/api/admin/users/:id', auth, owner, async (req, res) => {
    try {
        const user = await User.findById(req.params.id);
        if (!user)
            return res.status(404).send({ message: 'User not found' });
        if (user.role === 'owner')
            return res.status(400).send({ message: 'Cannot block admins' });
...
// Admin Stats
app.get('/api/admin/stats', auth, owner, async (req, res) => {
...
app.post('/api/cart/add', auth, async (req, res) => {
    try {
        ...
        const itemIndex = cart.items.findIndex((p) => p.product.toString() === productId && p.selectedSize === size && p.selectedColor === color);
        if (itemIndex > -1) {
