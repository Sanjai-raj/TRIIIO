...
async function connect() {
    await mongoose.connect(MONGO_URI, { dbName: DB_NAME, useNewUrlParser: true, useUnifiedTopology: true } as any);
...
async function up() {
    try {
        ...
        if (users && Array.isArray(users) && users.length) {
            ...
            const hashed = await Promise.all(users.map(async (u: any) => {
            ...
            console.log('Inserted users:', (users as any).length);
        }
        ...
        if (products && Array.isArray(products) && products.length) {
            ...
            console.log('Inserted products:', (products as any).length);
        }
        ...
        if (orders && Array.isArray(orders) && orders.length) {
            ...
            console.log('Inserted orders:', (orders as any).length);
        }
